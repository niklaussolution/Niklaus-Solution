...
export const createCertificate = async (req: Request, res: Response) => {
  try {
    ...
    if (!studentName || !studentEmail || !courseName || !completionDate || !dateOfBirth) {
      return res.status(400).json({
        message: 'Missing required fields: studentName, studentEmail, courseName, completionDate, dateOfBirth',
      });
    }
...
// Verify certificate using Certificate ID and Date of Birth
export const verifyCertificate = async (req: Request, res: Response) => {
  try {
    const { certificateId, dateOfBirth } = req.body;

    if (!certificateId || !dateOfBirth) {
      return res.status(400).json({
        message: 'Certificate ID and Date of Birth are required',
      });
    }
    ...
    // Verify DOB matches
    if (certData.dateOfBirth !== dateOfBirth) {
