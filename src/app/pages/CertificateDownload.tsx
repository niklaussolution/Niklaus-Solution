...
export const CertificateDownload: React.FC = () => {
  ...
  const handleVerifyAndDownload = async (e: React.FormEvent) => {
    ...
    if (!certificateId.trim() || !dateOfBirth.trim()) {
    ...
    try {
      ...
      // DOB comparison: try direct equality first, else compare Date timestamps when possible
      const providedDOB = dateOfBirth.trim();
      const storedDOB = (certDataRaw.dateOfBirth || '').toString().trim();

      const dobMatches = (() => {
        if (!storedDOB) return false;
        if (providedDOB === storedDOB) return true;
        try {
          const d1 = new Date(providedDOB);
          const d2 = new Date(storedDOB);
      ...
      if (!dobMatches) {
        setError('Invalid credentials (Date of birth does not match)');
        return;
      }
      ...
      setSuccess('Verified via Firestore — Generating PDF...');
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 mb-6">
          ...
          <p className="text-gray-600 text-center">
            Enter your Certificate ID and Date of Birth to download your certificate
          </p>
        </div>
          ...
          {!certificate ? (
            <form onSubmit={handleVerifyAndDownload} className="space-y-6">
              ...
              <p className="text-xs text-gray-500 text-center">
                These credentials are used to verify your identity and ensure secure access to your certificate.
              </p>
